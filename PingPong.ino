...
void displayResult() {
  ...
  if (scoreA == 0) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("restart the Game");
    while (true); // waiting for restart
...
void loop() {
  ...
  if (x == 5) {
    if (ly - 1 > y || y > ly + 4) {
      scoreA--;
      displayResult();
      x = 39; y = 8;//setting ball position
      xi *= -1; //reversing ball movement
      yi *= -1;
    }
    else {//changing ball movement according to point of contact on the strikerA (To avoid inevitability)
      if (ly - 1 == y  || ly + 4 == y ) {
        xi = 1;
        yi = 1;
      }
  ...
  //for collision ditection on strikerB side
    ...
    else {//changing ball movement according to point of contact on the strikerB (To avoid inevitability)
      if (ry - 1 == y  || ry + 4 == y ) {
        xi = 1;
        yi = 1;
      }
